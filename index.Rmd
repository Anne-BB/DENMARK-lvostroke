...
#save(hosp_addresses, file = "hosp_adresses.rda")
...
europeRGDK <- mutate(europeRGDK,
       ...
       #
       DirectDistanceToNearestHosp   = pmin(DirectDistanceToAarhus,
...
StrokeHosp <- filter(europeRGDK,
                                 DirectDistanceToNearestHosp < dist_range) %>%
        mutate(Postcode = as.numeric(COMM_ID)) %>%
        select(-starts_with("DK"))


p<-mapview(StrokeHosp,map.type="OpenStreetMap.HOT", alpha.regions=0.5, zcol="DirectDistanceToAarhus") +mapview(HospLocations, zcol="Center")+mapview(DKnuts2_sf2["strokenum"])
p
...
#set distance 10 km
#change to 200 km
dist_range <- units::set_units(90, km)
...
#then remove slash sign
library(readr)
denmarkstrokepdf <- read_csv("denmarkstrokepdf.csv")
save(denmarkstrokepdf, file= "denmarkstrokepdf.Rda")

#denmarkstrokepdf$strokeinc<-str_replace(denmarkstrokepdf$X3,"as.date(format=%d%b)","as.character(dates)")###did not work
denmarkstrokepdf$strokeinc<-str_replace(denmarkstrokepdf$X3,"3-Jun","3/6") %>%str_replace("Aug-43","8/43")%>%str_replace("Sep-23","9/23") %>%str_replace("Apr-14","4/14") %>%str_replace("4-May","4/5")

denmarkstrokepdf$strokenum<-str_replace(denmarkstrokepdf$strokeinc, "[0-9]*","")%>% str_replace("/","\\")

denmarkstrokepdf$Uoplyst<-str_replace(denmarkstrokepdf$Uoplyst,"Bornholms Regionskommune","Bornholm")
denmarkstrokepdf$Uoplyst<-str_replace(denmarkstrokepdf$Uoplyst,"Lyngby-Tårbæk","Lyngby-Taarbæk")
denmarkstrokepdf$Uoplyst<-str_replace(denmarkstrokepdf$Uoplyst, "Århus","Aarhus")


save(denmarkstrokepdf, file= "denmarkstrokepdf.Rda")

###Kommunemap
#Kommuner<-st_read("./DAGIREF_SHAPE_UTM32-EUREF89/ADM/KOMMUNE.shp")
#save(Kommuner, file="Kommuner.Rda")
#mapview(kommuner)
#View(Kommuner)
#Kommuner1<-select(Kommuner, starts_with("KOMNAVN"), starts_with("KOMKODE"))
#save(Kommuner1, file="Kommuner1.Rda")

#Kommuner.multi<-group_by(Kommuner,KOMKODE)%>%summarize(geometry=st_combine(geometry))
#save(Kommuner.multi, file="Kommuner.multi.Rda")
#Kommunerfinal<-st_join(Kommuner.multi, Kommuner1, by=c(KOMKODE))
#View(Kommunerfinal)

#Kommuner.multi1<-group_by(Kommuner,KOMNAVN)%>%summarize(geometry=st_combine(geometry))
#save(Kommuner.multi, file="Kommuner.multi1.Rda")
load("Kommuner.multi1.Rda")
###Kommuner.multi1 is the shapefile!


##join DemoKom (demografic) with denmarkstroke (strokedensity)
if(file.exists("DemoStroke.Rda")){load("DemoStroke.Rda")} else {

DemoStroke<-right_join(denmarkstrokepdf, Kommuner.multi1, by=c("Uoplyst" = "KOMNAVN"))
save(DemoStroke, file = "DemoStroke.Rda")}
View(DemoStroke)

DemoStroke_sf<-st_sf(DemoStroke)

#mapview(DemoStroke_sf, col.regions="strokenum")

library(tmap)
tmap_mode("view")


tm_shape(DemoStroke_sf)+tm_polygons("strokenum", palette="YlOrBr",title="Incidens af Stroke DK 2018")
